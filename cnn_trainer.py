...
(train_X, train_Y, train_Y_one_hot, class_names) = preprocess_images.load_dataset_from_file('train_data'+str(IMG_WIDTH)
    +'x' + str(IMG_HEIGHT)+ '.npy')
(test_X, test_Y, test_Y_one_hot, class_names_test) = preprocess_images.load_dataset_from_file('test_data'+str(IMG_WIDTH)
    +'x' + str(IMG_HEIGHT)+  '.npy')
...
model_name = "waste_model"+("_3d" if rgb else "") + ("_dropout" if dropout else "") + \
    ("_reg"+str(reg_lambda) if reg_lambda!=0 else "") +"_in" + str(IMG_WIDTH)+"x"+str(IMG_HEIGHT)
print("saved to:",model_name)
